...
performance_hyp_d = filtered_d %>% 
  select(-DoNotUseVideoGamePerformanceData, -starts_with("Game")) # your code here: remove the columns containing "Game" in the name
```
...
rating_hyp_long_d = rating_hyp_d %>%
  gather(Measurement, Rating, -c("Subject", "Cond"))## your code here
```
...
```{r echo=TRUE}
# make a IsRecall variable
# IsRecall is true if contains friends or strangers 
...
rating_summary_d = rating_hyp_long_d %>% 
  group_by(IsRecall, Emotion, ConfrontationalGame) %>%
